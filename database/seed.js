...
const price = faker.commerce.price(100, 180.0, 2);
...
const toFillListingItemsTable = function () {
  let listingId = 10001;
  let weekend;
  //set random number to set max num of Guests
  for (let i = 0; i < 100; i++) {
    //random price using Faker npm package
    let pricePerNight = faker.commerce.price(100, 180.0, 2); //range between 100 - 180
...
const makeCalendarDays = function () {
  let num;
  let isTrue = true;
...
const setUpSixBookingsPerListing = function (arr, listingId) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    let eachQuery = `INSERT into bookings (listingId, nights, month, checkIn, checkOut, guests, children, infants) VALUES (${listingId}, ${days}, '${startMonthSliced}', '${checkInDate}', '${checkOutDate}', ${guests}, 0, 0 ); \n `;
...
const toFillBookingsTable = function (arr, listingId) {
  for (let i = 0; i < 100; i++) {
...
const writeSchema = function (callback) {
